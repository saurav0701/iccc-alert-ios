...
struct CameraStreamSettingsView: View {
    ...
    @State private var selectedMode: MultiCameraOptimizer.OptimizationMode = .balanced
    ...
    var body: some View {
        List {
            Section(header: Text("Network Status")) {
                ...
                if bandwidthManager.isMeteredConnection {
                    HStack {
                        Image(systemName: "exclamationmark.triangle")
                            .foregroundColor(.orange)
                        Text("Metered Connection - Data Saver Recommended")
                            .font(.caption)
                    }
                }
            }
            ...
            Section(header: Text("Multi-Camera Performance")) {
                Picker("Optimization Mode", selection: $selectedMode) {
                    ForEach(MultiCameraOptimizer.OptimizationMode.allCases, id: \.self) { mode in
                        Text(mode.rawValue).tag(mode)
                    }
                }
                .onChange(of: selectedMode) { newMode in
                    optimizer.setOptimizationMode(newMode)
                }
                ...
                HStack {
                    ...
                    Text("\(String(format: "%.0f", optimizer.memoryUsageMB)) MB")
                        .foregroundColor(optimizer.isMemoryWarning ? .red : .secondary)
                }
            }
            
            Section(header: Text("Statistics")) {
                Button("View Stream Health Report") {
                    print(StreamHealthMonitor.shared.getAllStreamStatistics())
                }
                
                Button("View Bandwidth Report") {
                    print(bandwidthManager.getBandwidthReport())
                }
                
                Button("View Optimization Report") {
                    print(optimizer.getOptimizationReport())
                }
            }
        }
